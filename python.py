...
#device.send_event(str(event_timestamp_unix_ns))
#device.send_event(str(time.time_ns()))
device.send_event(event_name=str(time.time_ns()),event_timestamp_unix_ns=time.time_ns())
# now = datetime.now()
# name = str(now)
# time = name.split('.')
# name = name.split('.')[-1]
